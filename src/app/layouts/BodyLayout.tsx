...
export function BodyLayout({ children }: any) {
  ...
  for (let i = 0; i < 20; i++) {
    let percent = (i + 1) * 5;
    let currDarkColr =
      `color-mix(in srgb, ${darkStartColor} ${percent}%, ${darkStopColor})`;
    let currLightColr =
      `color-mix(in srgb, ${lightStartColor} ${percent}%, ${lightStopColor})`;
    darkColors.push(
      `color-mix(in srgb, ${tailwindDarkBGColor} ${tailwindDarkMixPercentage}%, ${currDarkColr}) ${percent}%`,
    );
    lightColors.push(
      `color-mix(in srgb, ${tailwindLightBGColor} ${tailwindLightMixPercentage}%, ${currLightColr}) ${percent}%`,
    );
  }
  ...
  const mainComponentStyle = {
    background: isDarkTheme
      ? `linear-gradient(${rotation}deg, ${darkColors[0]}, ${darkColors[1]}, ${
        darkColors[2]
      }, ${darkColors[3]}, ${darkColors[4]}, ${darkColors[5]}, ${
        darkColors[6]
      }, ${darkColors[7]}, ${darkColors[8]}, ${darkColors[9]}, ${
        darkColors[10]
      }, ${darkColors[11]}, ${darkColors[12]}, ${darkColors[13]}, ${
        darkColors[14]
      }, ${darkColors[15]}, ${darkColors[16]}, ${darkColors[17]}, ${
        darkColors[18]
      }, ${darkColors[19]}) no-repeat`
      : `linear-gradient(${rotation}deg, ${lightColors[0]}, ${
        lightColors[1]
      }, ${lightColors[2]}, ${lightColors[3]}, ${lightColors[4]}, ${
        lightColors[5]
      }, ${lightColors[6]}, ${lightColors[7]}, ${lightColors[8]}, ${
        lightColors[9]
      }, ${lightColors[10]}, ${lightColors[11]}, ${lightColors[12]}, ${
        lightColors[13]
      }, ${lightColors[14]}, ${lightColors[15]}, ${lightColors[16]}, ${
        lightColors[17]
      }, ${lightColors[18]}, ${lightColors[19]}) no-repeat`,
  };

  return (
    <div className={isDarkTheme ? "dark" : "light"}>
      <div
        className="gradient-animation min-h-screen"
        style={mainComponentStyle}
      >
        <div className="text-neutral-950 dark:text-neutral-50 flex flex-col min-h-screen items-stretch flex-grow crt">
          <header className="sticky top-0 backdrop-blur-sm px-2 py-2 md:px-1 blur-mobile z-10">
            <div className="flex flex-row justify-between mb-1 items-center">
              ...
              <div className="flex justify-between content-center items-center w-16 md:w-8">
                <button
                  className={(isMenuOn ? "is-active " : "") +
                    "hamburger hamburger--spin relative md:hidden"}
                  type="button"
                  onClick={handleMenu}
                >
                  <span
                    className={(isMenuOn ? "" : "hamhover ") +
                      "hamburger-box w-6 h-1"}
                  >
            ...
            <div>
              <ul
                className={(isMenuOn ? "h-8 " : "h-0 ") +
                  "md:hidden nav-bar flex list-none uppercase overflow-hidden show-menu transition-colors justify-center content-center items-center font-black"}
              >
                {[
                  { href: "/", text: "Início" },
                  { href: "/classes", text: "Aulas" },
              ...
              <hr
                className={(isMenuOn ? "active " : "") +
                  "block md:hidden h-1 transition-hr ease-in-out duration-500 bg-neutral-950 dark:bg-neutral-50 rounded-full"}
              />
            </div>
          </header>
