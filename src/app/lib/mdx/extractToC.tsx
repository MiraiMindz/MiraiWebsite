...
export async function getHeadings(source: string) {
  ...
  headingLines.map((raw: string) => {
    ...
    headersList.push(<li className="truncate list-disc transition-all ml-0 hover:ml-4 hover:text-violet-400 hover:dark:text-violet-600"><Link level={level} href={`#${headerLink}`}>{text}</Link></li>);
  });
